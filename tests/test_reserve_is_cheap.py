...
def _cfg(**kw):
    d = dict(capacity_kwh=10.0, soc_floor=10.0, eta_charge=1.0, round_trip_eff=1.0,
             reserve_cheap_band=0.20, reserve_anchor="trough")
    d.update(kw)
    return Config(**d)
...
def test_is_cheap_overnight_false_morning_true():
    ...
    ic = ctrl._build_is_cheap_by_hour(
        _slots([0.30, 0.43, 0.37, 0.30, 0.30, 0.20, 0.13, 0.14]), _cfg())
...
def test_reserve_monotone_non_increasing_across_overnight():
    ...
    assert all(a + 1e-9 >= b for a, b in zip(vals, vals[1:])), f"not monotone: {vals}"
