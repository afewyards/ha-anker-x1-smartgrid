...
def _cfg(**kw) -> Config:
    d = dict(capacity_kwh=10.0, soc_floor=20.0, soc_target=80.0, max_charge_w=3000.0, eta_charge=1.0)
    d.update(kw)
    return Config(**d)
...
def test_solar_cycle_end_idx_from_sun_times():
    now_h = datetime(2026, 6, 27, 14, 0, tzinfo=UTC)
    sun_times = (
        datetime(2026, 6, 27, 19, 0, tzinfo=UTC),  # today_sunset
        datetime(2026, 6, 28, 4, 0, tzinfo=UTC),  # tomorrow_sunrise = +14h
        datetime(2026, 6, 28, 19, 0, tzinfo=UTC),  # tomorrow_sunset
    )
...
def test_a_no_pre_solar_overcharge():
    ...
    common = dict(
        soc_start=30.0, cfg=cfg, window_start_h=0, window_len=6, terminal_mode="water_value", water_value=0.20
    )
...
def test_no_future_solar_ceiling_is_target():
    ...
    cfg = _cfg(round_trip_eff=1.0, cycle_cost_eur_per_kwh=0.04, max_export_w=3000.0, grid_export_limit_w=3000.0)
    n = 4
    pv = [0.0] * 4
    load = [0.0] * 4
...
def test_no_survival_grid_charge_on_no_export_night():
    ...
    res = optimize_grid(
        ...
        price,
        soc_start=80.0,
        cfg=cfg,  # high start: no floor breach
        window_start_h=0,
        window_len=6,
        terminal_mode="water_value",
        water_value=0.0,
...
def test_ceiling_prevents_afternoon_solar_spill_live_tariff():
    ...
    assert ceil_grid_kwh == pytest.approx(0.0, abs=0.1), f"CEILING grid_kwh={ceil_grid_kwh:.3f}"
    ...
    assert ceil_eur <= base_eur + 1e-3, f"Ceiling raised cost: base_eur={base_eur:.3f} ceil_eur={ceil_eur:.3f}"
...
def test_decide_state_stops_charging_at_ceiling():
    ...
    cfg = Config(
        capacity_kwh=10.0, soc_floor=5.0, soc_target=97.0, max_charge_w=6000.0, eta_charge=0.92, min_dwell_min=0
    )
...
def test_build_plan_horizon_caps_grid_at_ceiling_then_solar_fills():
    ...
    cfg = Config(capacity_kwh=10.0, soc_floor=5.0, soc_target=97.0, max_charge_w=6000.0, eta_charge=0.92)
    base = datetime(2026, 6, 27, 11, tzinfo=UTC)
