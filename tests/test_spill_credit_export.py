...
def _cfg(**kw):
    d = dict(
        capacity_kwh=10.0, soc_floor=5.0, soc_target=100.0, max_charge_w=6000.0,
        eta_charge=0.92, round_trip_eff=0.85, cycle_cost_eur_per_kwh=0.04,
        export_fee_eur_per_kwh=0.02, max_export_w=6000.0, grid_export_limit_w=6000.0,
        export_peak_band_frac=0.1, export_peak_lookback_h=4, enable_export=True,
    )
    d.update(kw)
    return Config(**d)
...
def test_spill_credit_is_decision_neutral():
    ...
    common = dict(
        soc_start=98.0, cfg=cfg, window_start_h=0, window_len=n,
...
def test_sunny_next_day_sells_first_day_surplus():
    ...
    ep = [effective_export_price(p, cfg) for p in raw_export]
    res = optimize_grid(
        pv, load, price, soc_start=90.0, cfg=cfg, window_start_h=0, window_len=n,
