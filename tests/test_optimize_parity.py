...
def assert_parity(opt: dict, hind: dict, *, tol: float = 1e-6, label: str = "") -> None:
    ...
    assert opt_inf == hind_inf, f"{prefix}infeasible mismatch: optimize_grid={opt_inf} hindsight={hind_inf}"
    ...
    assert len(opt["schedule"]) == 24, f"{prefix}optimize_grid schedule len {len(opt['schedule'])} != 24"
    assert len(hind["schedule"]) == 24, f"{prefix}hindsight schedule len {len(hind['schedule'])} != 24"
    for h in range(24):
...
class TestParityNearInfeasible:
    ...
    def test_target_unreachable_low_rate(self):
        ...
        opt, hind = call_both(pv, load, price, soc_start=20.0, cfg=cfg)
        assert bool(opt.get("infeasible", False)), "optimize_grid should flag infeasible"
        assert bool(hind.get("infeasible", False)), "hindsight should flag infeasible"
    ...
    def test_all_paths_blocked_pathological(self):
        ...
        price = [0.20] * 24
        opt, hind = call_both(pv, load, price, soc_start=20.0, cfg=cfg)
        assert bool(opt.get("infeasible", False)), "optimize_grid should flag infeasible (reserve target unreachable)"
        assert bool(hind.get("infeasible", False)), "hindsight should flag infeasible (reserve target unreachable)"
    ...
    def test_borderline_feasible_then_infeasible(self):
        ...
        assert not bool(opt_ok.get("infeasible", False)), "300W should be feasible"
        ...
        assert bool(opt_nok.get("infeasible", False)), "200W should be infeasible"
        assert bool(hind_nok.get("infeasible", False)), "200W hindsight should be infeasible"
...
class TestParityRandomDays:
    ...
    def _random_day(self, rng: random.Random) -> tuple:
        ...
        pv = [0.0] * 6 + [rng.uniform(0.0, 3.0) for _ in range(10)] + [0.0] * 8
...
def assert_export_parity(opt, hind, *, tol=1e-6, label=""):
    ...
    assert opt["export_revenue_eur"] == pytest.approx(hind["export_revenue_eur"], abs=tol), (
        f"[{label}] export_revenue_eur"
    )
    for h in range(24):
...
class TestSolarReservationParity:
    ...
    def test_ceiling_parity_solar_rich(self):
        ...
        day = DayData(pv_kwh=tuple(pv), load_kwh=tuple(load), price=tuple(price), soc_start=20.0)
        hind = hindsight_optimal_grid(day, cfg, grid_charge_ceiling=ceil)
        opt = optimize_grid(
            pv, load, price, soc_start=20.0, cfg=cfg, window_start_h=0, window_len=24, grid_charge_ceiling=ceil
        )
    ...
    def test_ceiling_with_reserve_and_export_parity(self):
        ...
        hind = hindsight_optimal_grid(
            day, cfg, export_price=export_price, reserve_by_hour=reserve, grid_charge_ceiling=ceil
        )
        opt = optimize_grid(
            pv,
            load,
...
class TestChargeMarginEurPerKwh:
    ...
    def test_margin_blocks_marginal_charge_but_not_profitable_arbitrage(self):
        ...
        assert res_b["schedule"][0] > 0.0, "profitable trough charge should still clear the margin"
...
class TestIdleDrainW:
    ...
    def test_idle_drain_lowers_passive_soc_trajectory(self):
        ...
        assert soc_on < soc_off, (
            f"idle_drain_w=130 should lower the resulting SoC vs idle_drain_w=0 (soc_on={soc_on}, soc_off={soc_off})"
        )
...
def test_optimize_grid_accepts_eta_curve_kwarg_none_is_identical():
    ...
    load = [1.0] * 24
    price = [0.20] * 24
    base = optimize_grid(pv, load, price, soc_start=60.0, cfg=cfg, window_start_h=0, window_len=24)
    with_none = optimize_grid(pv, load, price, soc_start=60.0, cfg=cfg, window_start_h=0, window_len=24, eta_curve=None)
...
class TestCurveOnMatchedPair:
    ...
    def _curve(self, cfg):
        ...
        charge = [
            BinStat(b.lo_w, b.hi_w, "charge", 0.85 + 0.02 * i, 0.85, 99, 9.0, True, "")
            for i, b in enumerate(base._charge)
        ]
        discharge = [
            BinStat(b.lo_w, b.hi_w, "discharge", 0.80 + 0.03 * i, 0.80, 99, 9.0, True, "")
    ...
    def test_charge_leg_zero_regret(self):
        ...
        opt = optimize_grid(pv, load, price, soc_start=60.0, cfg=cfg, window_start_h=0, window_len=24, eta_curve=curve)
...
def _two_bin_discharge_curve(cfg, *, lo_eta, hi_eta, split_w):
    ...
    disc = [
        BinStat(lo, hi, "discharge", (lo_eta if lo < split_w else hi_eta), None, 5, 1.0, True, "measured")
        for lo, hi in zip(los, his)
    ]
    chg = [BinStat(lo, hi, "charge", cfg.eta_charge, None, 5, 1.0, True, "measured") for lo, hi in zip(los, his)]
    return EfficiencyCurve(chg, disc, cfg.eta_charge, min(cfg.round_trip_eff / cfg.eta_charge, 1.0))
...
class TestExportEtaCurveParity:
    ...
    def test_export_leg_eta_curve_bin_crossing_parity(self):
        ...
        hind = hindsight_optimal_grid(
            day, cfg, export_price=export_price, terminal_mode="water_value", water_value=0.0, eta_curve=curve
        )
        opt = optimize_grid(
            pv,
            load,
...
def test_parity_holds_across_live_cfg_permutations(overrides, dt_h):
    ...
    day = DayData(pv_kwh=tuple(pv), load_kwh=tuple(load), price=tuple(price), soc_start=80.0)
    hind = hindsight_optimal_grid(
        day, cfg, export_price=export_price, terminal_mode="water_value", water_value=0.0, dt_h=dt_h
    )
    opt = optimize_grid(
        pv,
        load,
