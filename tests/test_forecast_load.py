...
NOW = datetime(2026, 6, 22, 12, 0, tzinfo=UTC)  # Monday
...
def test_profile_averages_by_hour_and_daytype():
    samples = [
        _s(datetime(2026, 6, 20, 8, 0, tzinfo=UTC), 400.0),  # Sat
        _s(datetime(2026, 6, 21, 8, 0, tzinfo=UTC), 600.0),  # Sun
        _s(datetime(2026, 6, 19, 8, 0, tzinfo=UTC), 1000.0),  # Fri (weekday)
    ]
...
def test_profile_respects_lookback():
    old = _s(datetime(2026, 1, 1, 8, 0, tzinfo=UTC), 999.0)
    recent = _s(datetime(2026, 6, 20, 8, 0, tzinfo=UTC), 400.0)
...
def test_predict_uses_fallback_when_missing():
    prof = {(False, 8): 800.0}
    when = datetime(2026, 6, 22, 9, 0, tzinfo=UTC)  # Monday hour 9 missing
    assert forecast.predict_load_w(prof, when, fallback_w=350.0) == 350.0
    when8 = datetime(2026, 6, 22, 8, 0, tzinfo=UTC)
...
def test_load_predictor_bare_fallback_with_quantile():
    ...
    when = datetime(2026, 6, 22, 10, 0, tzinfo=UTC)
...
def test_load_predictor_profile_p50_regression():
    ...
    """
    samples = [
        (datetime(2026, 6, 20, 8, 0, tzinfo=UTC).isoformat(), 400.0),  # Sat
        (datetime(2026, 6, 21, 8, 0, tzinfo=UTC).isoformat(), 600.0),  # Sun
    ]
    lp = forecast.LoadPredictor.from_profile_samples(samples, lookback_days=14, now=NOW)
    when = datetime(2026, 6, 22, 8, 0, tzinfo=UTC)  # Monday, hour 8 — weekend data exists
    when_sat = datetime(2026, 6, 20, 8, 0, tzinfo=UTC)  # Saturday
    when_mon_h9 = datetime(2026, 6, 22, 9, 0, tzinfo=UTC)  # Monday, hour 9 — no data at all
