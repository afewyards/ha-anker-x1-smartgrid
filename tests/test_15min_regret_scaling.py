...
def _run_with_spies(ctrl, day, ts_now):
    ...
    with patch(
        ...
        "custom_components.anker_x1_smartgrid.optimize.optimize_grid", side_effect=_spy_optimize_grid
    ):
...
def test_regret_scorer_scales_energy_and_threads_dt_h_at_15min():
    ...
    for call in captured["hindsight_optimal_grid"]:
        assert call["dt_h"] == pytest.approx(_dt_h), (
            f"hindsight_optimal_grid got dt_h={call['dt_h']}, want {_dt_h}"
        )
    for call in captured["realized_grid_cost"]:
        assert call["dt_h"] == pytest.approx(_dt_h), (
            f"realized_grid_cost got dt_h={call['dt_h']}, want {_dt_h}"
        )
    for call in captured["optimize_grid"]:
        assert call["dt_h"] == pytest.approx(_dt_h), (
            f"optimize_grid (shadow DP) got dt_h={call['dt_h']}, want {_dt_h}"
        )
