...
def _inputs():
    ...
    load  = [1.0]*24
    price = [0.30,0.28,0.25,0.22,0.20,0.18,0.22,0.28,0.32,0.35,0.33,0.31,
             0.29,0.27,0.25,0.28,0.32,0.35,0.38,0.40,0.36,0.32,0.28,0.25]
    return pv, load, price

def test_optimize_grid_explicit_dt_h_one_matches_default():
    pv, load, price = _inputs(); cfg = _cfg()
    base = optimize_grid(pv, load, price, soc_start=60.0, cfg=cfg,
                         window_start_h=0, window_len=24, export_price=price)
    seam = optimize_grid(pv, load, price, soc_start=60.0, cfg=cfg,
                         window_start_h=0, window_len=24, export_price=price, dt_h=1.0)
    assert seam == base
