...
def test_build_pv_curve_from_watts_bell_shape():
    ...
    assert 10 <= max_t.hour < 16, (
        f"Peak expected between 10:00-16:00 UTC (midday), got {max_t.hour}:00 UTC"
    )
    ...
    assert curve[-1][1] < max_w * 0.05, (
        f"Last bucket {curve[-1][0].hour}:00 UTC has {curve[-1][1]:.1f}W — "
        f"expected ~0 at end of solar window"
    )

    # The watts sequence is NOT monotonically non-decreasing — it must decrease after peak
    watts = [w for _, w in curve]
    assert not all(a <= b for a, b in zip(watts, watts[1:])), (
...
def test_build_pv_curve_from_watts_now_slicing():
    ...
    assert abs(pts[_dt(9, 0)] - 550.0) < 1.0, (
        f"Expected mean 550W at 09:00, got {pts[_dt(9, 0)]}"
    )
...
async def test_read_pv_today_watts_multi_string_sums_per_timestamp(hass):
    ...
    assert abs(pts[t1130] - 900.0) < 1.0, (
        f"Expected 700+200=900W at 11:30Z (sum), got {pts[t1130]}"
    )
...
def test_build_pv_curve_from_watts_contiguous_hourly_no_overnight_gap():
    ...
    while h <= last:
        assert h in pts, (
            f"Missing hourly point at {h} — overnight gap was not filled with 0.0"
        )
        h += timedelta(hours=1)

    # Overnight hours (21:00 06-27 → 05:00 06-28) must be exactly 0.0
    overnight_hours = [
        datetime(2026, 6, 27, h_, 0, tzinfo=UTC) for h_ in range(21, 24)
    ] + [
    ...
    for t in overnight_hours:
        if first <= t <= last:
            assert pts[t] == 0.0, (
                f"Overnight hour {t} should be 0.0W, got {pts[t]}"
            )
