...
def _row(day, hour, load, temp, weekend=False):
    ts = datetime(2026, 6, day, hour, 0, tzinfo=UTC)
...
def test_fit_and_predict_specific_cell():
    ...
    cold = m.predict_load_w(datetime(2026, 6, 22, 8, tzinfo=UTC), temp=2.0, fallback_w=0.0)
    warm = m.predict_load_w(datetime(2026, 6, 22, 8, tzinfo=UTC), temp=18.0, fallback_w=0.0)
...
def test_predict_falls_back_to_hour_cell_when_temp_bucket_missing():
    ...
    out = m.predict_load_w(datetime(2026, 6, 22, 8, tzinfo=UTC), temp=19.0, fallback_w=0.0)
    assert out == 1000.0


def test_predict_uses_fallback_when_no_data():
    m = loadmodel.BucketedLoadModel.fit([])
    out = m.predict_load_w(datetime(2026, 6, 22, 8, tzinfo=UTC), temp=10.0, fallback_w=375.0)
    assert out == 375.0
...
def test_bucketed_p80_returns_empirical_quantile():
    ...
    m = loadmodel.BucketedLoadModel.fit(rows)
    when = datetime(2026, 1, 5, 8, tzinfo=UTC)  # Monday
...
def test_bucketed_p80_sparse_cell_falls_back_to_hourly():
    ...
    m = loadmodel.BucketedLoadModel.fit(rows)
    when = datetime(2026, 1, 5, 8, tzinfo=UTC)  # Monday (weekday)
...
def test_bucketed_p50_path_unchanged_regression():
    ...
    m = loadmodel.BucketedLoadModel.fit(rows)
    when = datetime(2026, 6, 22, 8, tzinfo=UTC)
