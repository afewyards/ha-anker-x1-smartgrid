...
def test_charge_cap_is_quarter_at_dt_025():
    ...
    load = [0.0] * n
    price = [0.05] + [0.40] * (n - 1)
    out = optimize_grid(pv, load, price, soc_start=20.0, cfg=_cfg(), window_start_h=0, window_len=n, dt_h=0.25)
...
def test_optimize_hindsight_mirror_holds_at_dt_025():
    ...
    opt = optimize_grid(
        pv, load, price, soc_start=50.0, cfg=_cfg(), window_start_h=0, window_len=n, dt_h=0.25, export_price=price
    )
