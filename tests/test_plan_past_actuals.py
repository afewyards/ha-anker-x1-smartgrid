...
def _slot(h, price=0.20):
    return PriceSlot(start=datetime(2026, 6, 29, h, tzinfo=UTC), price=price)
...
def test_past_slot_uses_actuals_and_leaves_forward_soc_unchanged():
    ...
    intervals = [
        ForecastInterval(datetime(2026, 6, 29, 10, tzinfo=UTC), 1000.0, 300.0, 1.0),
        ForecastInterval(datetime(2026, 6, 29, 11, tzinfo=UTC), 1200.0, 300.0, 1.0),
    ]
    past = {
        ...
        datetime(2026, 6, 29, 9, tzinfo=UTC): {
            "pv_w": 0.0,
            "load_w": 400.0,
            "soc": 28.0,
            "solar_charge_w": 0.0,
...
def test_future_slot_kwh_matches_watts_times_dt_h():
    cfg = _cfg()
    now_h = datetime(2026, 6, 29, 10, tzinfo=UTC)
    slots = [_slot(10), _slot(11)]
    intervals = [
        ForecastInterval(now_h, 1000.0, 300.0, 1.0),
        ForecastInterval(datetime(2026, 6, 29, 11, tzinfo=UTC), 1200.0, 300.0, 1.0),
    ]
    edge = datetime(2026, 6, 29, 12, tzinfo=UTC)
...
def test_future_grid_charge_and_export_kwh():
    ...
    h10 = datetime(2026, 6, 29, 10, tzinfo=UTC)
    h11 = datetime(2026, 6, 29, 11, tzinfo=UTC)
    slots = [_slot(10), _slot(11)]
    intervals = [
