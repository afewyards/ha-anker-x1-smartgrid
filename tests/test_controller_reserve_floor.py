...
def test_reserve_by_hour_spans_to_tomorrow_pickup_not_price_horizon():
    ...
    intervals_reserve = [ForecastInterval(NOW + timedelta(hours=i), 0.0, 500.0, 1.0) for i in range(8)] + [
...
def test_reserve_floored_at_firmware_when_load_tiny():
    cfg = _cfg()
    slots = [PriceSlot(NOW + timedelta(hours=i), 0.20) for i in range(4)]
    intervals_reserve = [ForecastInterval(NOW + timedelta(hours=i), 0.0, 50.0, 1.0) for i in range(3)] + [
...
NOW18 = datetime(2026, 6, 26, 18, 0, tzinfo=UTC)  # 18:00 — evening export peak
...
def _overnight_intervals():
    ...
    return [ForecastInterval(NOW18 + timedelta(hours=i), 0.0, 500.0, 1.0) for i in range(12)] + [
...
SUMMER_NOW = datetime(2026, 6, 26, 19, 0, tzinfo=UTC)  # summer evening
SUMMER_SUNSET = datetime(2026, 6, 26, 21, 0, tzinfo=UTC)
SUMMER_SUNRISE = datetime(2026, 6, 27, 5, 0, tzinfo=UTC)
SUMMER_SUNSET2 = datetime(2026, 6, 27, 21, 0, tzinfo=UTC)
...
def test_trough_anchor_lowers_reserve_vs_legacy_when_cheap_ahead():
    ...
    ivs = [ForecastInterval(NOW + timedelta(hours=i), 0.0, 500.0, 1.0) for i in range(3)] + [
