...
def _make_cfg() -> Config:
    """Config whose ``soc_floor`` (20% = 2 kWh) genuinely binds on the test day.
    ...
    return Config(
        capacity_kwh=10.0,
        soc_floor=20.0,   # 2 kWh firmware floor
...
def _drain_to_floor_day() -> DayData:
    ...
    Trajectory under a zero charge schedule:
      h0:  3 -> 2  (== floor)                import 0
      h1..h23: 2 -> 1 -> clamp to floor 2    import 1.0 kWh each (23 hours)
    -> 23 kWh of below-floor direct grid->load import @ 0.20 €/kWh = 4.60 €.
    """
    pv = [0.0] * 24
    load = [1.0] * 24
    price = [0.20] * 24
...
def test_oracle_kwh_folds_floor_import_no_phantom_over_buy():
    ...
    # The floor binds for 23 hours, each a 1 kWh direct import @ 0.20 €.
    assert realized["kwh"] == pytest.approx(23.0, abs=1e-9)
    assert realized["eur"] == pytest.approx(4.60, abs=1e-9)
    ...
    # phantom over-buy.  Pre-fix both were inflated by the 23 kWh floor import.
    ...
    assert optimal["kwh"] == pytest.approx(23.0, abs=1e-9)
