...
class DataRecorder:
    ...
    def read_soc_samples(self, since_iso: str | None = None) -> list[tuple[str, float, str | None]]:
        """Return (ts, soc, state) rows ordered by ts ascending, NULL soc skipped.
        ...
        ``state`` rides along because a dwell is only a dwell if WE commanded
        the charge -- SoC alone cannot tell a forced hold from a sunny
        afternoon's passive plateau. It is NOT NULL-filtered: a NULL state is
        simply not FORCING, which is the fail-closed answer.
        """
        with self._lock:
            if since_iso is not None:
                cur = self._conn.execute(
                    "SELECT ts, soc, state FROM samples WHERE soc IS NOT NULL AND ts >= ? ORDER BY ts ASC",
                    (since_iso,),
                )
            else:
                cur = self._conn.execute("SELECT ts, soc, state FROM samples WHERE soc IS NOT NULL ORDER BY ts ASC")
            return [(ts, float(soc), state) for ts, soc, state in cur.fetchall()]
