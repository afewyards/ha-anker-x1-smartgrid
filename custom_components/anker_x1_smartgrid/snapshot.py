...
def occ_status_attrs(
    ...
    return {
        ...
        "occ_state_now": (
            min(occupancy.STATE_MAX, max(0, int(persons_home_now))) if persons_home_now is not None else None
        ),
        "occ_expected_state": (occ_table.climo_state.get(occupancy.band_of(now)) if occ_table is not None else None),
        ...
        "occ_cells_ready": (occ_table.cells_ready if occ_table is not None else 0),
    }
    ...
    *,
    ...
    return {
        ...
        **occ_status_attrs(
        ...
        "battery_net_today_eur": round(today_export_revenue_eur - today_charge_cost_eur, 6),
