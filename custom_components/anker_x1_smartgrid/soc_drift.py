...
def expected_soc_delta_kwh(
    forecast_pv_w: float,
    forecast_load_w: float,
    dt_h: float,
    eta_charge: float,
    eta_discharge: float,
    idle_drain_w: float = 0.0,
) -> float:
...
def per_step_drift_kwh(expected_dc_kwh: float, measured_dc_kwh: float, intentional_export_dc_kwh: float = 0.0) -> float:
...
def accumulate(prev_kwh: float, per_step_kwh: float, *, dt_h: float = 0.0, halflife_h: float = 0.0) -> float:
...
def reset_if_new_day(accumulator_kwh: float, stored_day: str | None, today_key: str) -> tuple[float, str]:
...
def drift_kwh(
    accumulator_kwh: float, engage_deadband_kwh: float, release_deadband_kwh: float, engaged_prev: bool
) -> tuple[float, bool]:
